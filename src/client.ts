...
export interface ClientOptions {
   ...
   *
   * Defaults to process.env['ARVK_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['ARVK_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the Arvk API.
 */
export class Arvk {
  apiKey: string;
   ...
   * @param {string} [opts.baseURL=process.env['ARVK_BASE_URL'] ?? https://petstore3.swagger.io/api/v3] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('ARVK_BASE_URL'),
    apiKey = readEnv('PETSTORE_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.ArvkError(
        "The PETSTORE_API_KEY environment variable is missing or empty; either provide it, or instantiate the Arvk client with an apiKey option, like new Arvk({ apiKey: 'My API Key' }).",
      );
    }
    ...
    this.timeout = options.timeout ?? Arvk.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('ARVK_LOG'), "process.env['ARVK_LOG']", this) ??
      defaultLogLevel;
    ...
    const customHeadersEnv = readEnv('ARVK_CUSTOM_HEADERS');
    if (customHeadersEnv) {
  ...
  static ArvkError = Errors.ArvkError;
